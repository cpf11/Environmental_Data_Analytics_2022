...
ggplot(BRA_complete) +
  ...
  geom_smooth(aes(y = Rainfall, x = Date), color = "#c13d75ff")
...
Rainfall_ts <- ts(Rainfall_BR_processed$Rainfall, start = c(fyear,fmonth), frequency = 12)
Temp_ts <- ts(Temp_BR_processed$Temperature_C, start = c(fyear,fmonth), frequency = 12)
...
Rainfall_stl <- stl(Rainfall_ts, s.window = "periodic")
...
# components of each series and turn them into data frames.#because output of decomposition is time series data

Rainfall_Components <- as.data.frame(Rainfall_stl$time.series[,1:3])
...
Temp_Components <- as.data.frame(Temp_stl$time.series[,1:3])
