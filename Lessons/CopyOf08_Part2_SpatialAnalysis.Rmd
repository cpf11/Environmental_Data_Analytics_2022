...
wind_raw <- read.csv(
  ...
  colClasses = c('HUC 12' = 'factor')) %>% 
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% #start at first character and go 8 characters down 
  group_by(HUC_8) %>%
...
triangle_HUCs<-huc8_sf_utm %>%
  filter(st_insersects(x=., y=triangle_co, sparse=F))
