...
#Exercise 2:
#1
EPA.ozone.year <- ggplot(EPAair, aes(y = Ozone/Year)) +
  geom_density()
...
ex3<- lm(EPAair$PM2.5~EPAair$Ozone)
#or
...
EPAairCorr <- cor(EPAair.subset) #Matrix with all the correlations 
...
TPAIC <- lm(data = EPAair, PM2.5 ~ Ozone + meanLat + 
              meanLng + Month + Year)

step(TPAIC)
EPAmodel <- lm(formula = PM2.5 ~ Ozone + meanLat + meanLng + Month, data = EPAair)
summary(EPAmodel)
