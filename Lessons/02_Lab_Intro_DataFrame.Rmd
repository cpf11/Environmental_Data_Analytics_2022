...
##data frame can have values of different nature, e.g. numbers and characters, compared to a matrix that can only have one type of values
##need to have the same number of rows
...
#Option 1
df <- cbind(df_car_speed,car_dist)  #similarly rows could be added using rbind() #only works if same number of rows
class(df)
df
...
<<<<<<< HEAD
df[,1] #after comma is columns #before comma is rows 
=======
df_2 <- df[1:10,1:2]
